...
class GaborFilter {
 public:
  explicit GaborFilter(const EncoderConfig &config);
 ...
 private:
  ...
  void GetKernelReal(cv::Mat *kernel, int width, int height, int dimension, int direction, double kmax = CV_PI / 2,
                     double f = sqrt(2.0), double sigma = 2 * CV_PI) const;
  void PreProcessImage(const cv::Mat &src, cv::Mat *result) const;
  void EnhanceImage(const cv::Mat &src, cv::Mat *result) const;
