...
Status ConfigReader::SetGaborKernelSize(uint8_t size) {
  ...
  cfg_.gabor_kernel_size = size;
  return Status::Ok();
}

Status ConfigReader::SetLaplaceKernelSize(uint8_t size) {
  if (size > cfg_.image_size || size > limit::kMaxLaplaceKernelSize) {
...
Status SimpleConfigReader::Load() {
  INVOKE_FUNC_WITH_STATUS(SetImageSize(cfg_.image_size));
  INVOKE_FUNC_WITH_STATUS(SetGaborKernelSize(cfg_.gabor_kernel_size));
  INVOKE_FUNC_WITH_STATUS(SetLaplaceKernelSize(cfg_.laplace_kernel_size));
  INVOKE_FUNC_WITH_STATUS(SetGaborDirecions(cfg_.gabor_directions));
  return Status::Ok();
}
